...
function Header() {
  ...
  useEffect(() => {
    ...
    if (sessionStorage.getItem("existingUser")) {
      ...
      const exisitingProfile = JSON.parse(sessionStorage.getItem("existingUser"))
      setUserProfile(exisitingProfile.profile)
    }
          ...
          {!token ?
            ...
            <div className='relative inline-block text-left'>
              <button onClick={() => setDropdownStatus(!dropdownStatus)} className='w-full flex items-center bg-white px-3 py-3 shadow-lg hover:bg-gray-100'>
                ...
                <img src={userProfile == "" ? "https://static.vecteezy.com/system/resources/previews/018/765/757/non_2x/user-profile-icon-in-flat-style-member-avatar-illustration-on-isolated-background-human-permission-sign-business-concept-vector.jpg"
                  : userProfile.startsWith("https") ? userProfile : `${SERVERURL}/imageuploads/${userProfile}`
                }
                  width={"50px"} height={"50px"} alt="" style={{ borderRadius: '50%' }} />
              ...
              {dropdownStatus &&
                <div className='absolute right-0 z-10 mt-2 w-40 origin-top-right rounded-md bg-white shadow-lg'>
                  ...
                  <button type='button' onClick={handleLogOut} className='block px-4 py-2 text-sm text-gray-70'>Logout</button>
                </div>}
            ...
            <div className='relative inline-block text-left'>
              ...
              {dropdownStatus &&
                <div className='absolute right-0 z-10 mt-2 w-40 origin-top-right rounded-md bg-white shadow-lg'>
                  ...
                  <button type='button' onClick={handleLogOut} className='block px-4 py-2 text-sm text-gray-800'>Logout</button>
                </div>
              }
            </div>
          )}
